  ...
  return (
    ...
    >
      <div className="max-w-2xl mx-auto">
        <div className="bg-slate-900/20 border border-slate-700/50 rounded-lg shadow-lg p-8">
          {lastAttempt && (
            <div className="mb-6 p-4 bg-slate-800/50 border border-slate-700/50 rounded-lg">
              ...
              {typeof lastAttempt.score === 'number' && (
                <span className="ml-2 text-slate-300">Pontuação: {lastAttempt.score.toFixed(0)}%</span>
              )}
            </div>
          )}
        {!result ? (
          <form onSubmit={e => { e.preventDefault(); handleSubmit(); }}>
            ...
            <div className="flex justify-end gap-2 mt-4">
              <Button type="submit" className="bg-emerald-600 hover:bg-emerald-700 text-white" disabled={submitting}>Enviar Respostas</Button>
            </div>
          </form>
        ) : (
          <div className="flex flex-col items-center gap-4 py-6">
            ...
            {result.passed && (
              <Button className="mt-2 bg-emerald-600 hover:bg-emerald-700 text-white" onClick={handleNext}>Próxima Aula</Button>
            )}
          </div>
        )}
        </div>
